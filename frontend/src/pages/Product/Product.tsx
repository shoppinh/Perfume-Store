...
const Product: FC<RouteComponentProps<{ id: string }>> = ({ match }) => {
  ...
  const perfume: Partial<Perfume> = useSelector(
    (state: AppStateType) => state.perfume.perfume
  );
  const reviews: Array<Review> = useSelector(
    (state: AppStateType) => state.perfume.reviews
  );
  ...
  const addReview = (event: FormEvent<HTMLFormElement>): void => {
    ...
    const review: ReviewData = {
      perfumeId: match.params.id as string,
      author,
      message,
      rating,
    };
            ...
            <div className="col-md-7">
              ...
              <div className="row">
                <div className="col-md-2">
                  {renderStars(
                    perfume.perfumeRating === 0 ? 5 : perfume.perfumeRating
                  )}
                </div>
